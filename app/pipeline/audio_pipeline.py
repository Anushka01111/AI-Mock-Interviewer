...
import os
# numba's JIT compiler (used internally by librosa for speed) requires
# llvmlite.dll, which is blocked by a Windows Application Control policy
# on this machine. Disabling JIT falls back to pure Python/NumPy — slightly
# slower, but fully functional, and avoids the blocked DLL entirely.

# Whisper internally shells out to a system 'ffmpeg' command for audio
# decoding. We don't rely on ffmpeg being separately installed system-wide —
# instead we point Whisper at the ffmpeg binary bundled with imageio-ffmpeg
# by adding its folder to PATH for this process only. This keeps deployment
# simple: no need to install ffmpeg separately on any machine this runs on.
# Copy imageio-ffmpeg's bundled binary to a folder with the exact filename
# 'ffmpeg.exe' that Whisper's subprocess call expects (Whisper hardcodes
# the command name "ffmpeg", and imageio-ffmpeg's binary has a different
# filename like 'ffmpeg-win-x86_64-v7.1.exe', so shutil.which('ffmpeg')
# would otherwise fail even with the right folder on PATH).
import shutil
import imageio_ffmpeg

_ffmpeg_bin_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bin")
_ffmpeg_target = os.path.join(_ffmpeg_bin_dir, "ffmpeg.exe")

os.makedirs(_ffmpeg_bin_dir, exist_ok=True)
if not os.path.exists(_ffmpeg_target):
    shutil.copy(imageio_ffmpeg.get_ffmpeg_exe(), _ffmpeg_target)
