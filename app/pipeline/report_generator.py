...
try:
    ...
    print(f"[WARNING] matplotlib not available ({e}). Charts will be skipped in PDF reports.")
...
def build_pdf_report(
    ...
    radar_result = generate_radar_chart(session_report["session_summary"], radar_path)
    bar_result = generate_accuracy_bar_chart(session_report["per_question"], bar_path)
...
def build_pdf_report_from_data(
    session_report: dict,
    feedback: dict,
    output_pdf_path: str,
    charts_dir: str = "data/generated_reports/charts"
):
    """
    Same as build_pdf_report, but takes session_report and feedback as
    live Python dicts instead of reading them from JSON files on disk.
    Used by the live API endpoint.
    """
    os.makedirs(charts_dir, exist_ok=True)
    radar_path = os.path.join(charts_dir, f"radar_{output_pdf_path.split('/')[-1]}.png")
    bar_path = os.path.join(charts_dir, f"bar_{output_pdf_path.split('/')[-1]}.png")
